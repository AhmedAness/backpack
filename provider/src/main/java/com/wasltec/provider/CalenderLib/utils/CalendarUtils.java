package com.wasltec.provider.CalenderLib.utils;
...
public class CalendarUtils {
    ...
    public static boolean is_in_between (Calendar dateCalendar,Date start,Date end ) {
        if (start.compareTo(end)<=0)
        {
            if (dateCalendar.getTime().compareTo(start)<=0)
            {            return false;        }
            else
            {
                if (dateCalendar.getTime().compareTo(end)<=0)
                {                return true;            }
                else
                {                return false;            }
            }
